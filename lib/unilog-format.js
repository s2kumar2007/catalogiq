...
function attributeTriples(product, extraAttributes) {
  const attrs = [
  ...
  for (const attr of extraAttributes || []) {
    attrs.push([attr.label, attr.value, attr.uom || ""]);
  }
  return attrs.filter(([, value]) => value).slice(0, 50);
}
...
function buildUnilogDeliveryRecord(inputRow, options = {}) {
  ...
  const attrs = attributeTriples(product, options.extraAttributes);
  attrs.forEach(([label, value, uom], i) => {
    const n = i + 1;
  ...
  return {
    ...
    trace: {
      ...
      confidence: {
        ...
        attributes: attrs.length / 7,
      },
      needs_human_review: category.needsReview || !manufacturer || !brand || !options.officialSourceData,
    },
  };
}
